...
const VideoCapturePage = () => {
  ...
  return (
    ...
    {!recordModal&&!recording&&<button className="text-white uppercase min-w-40 bg-blue-600 p-3 rounded flex flex-row items-center font-semibold shadow-xl" onClick={()=>setRecordModal(true)}>start Record <IoIosVideocam  className="text-2xl ml-3"/> </button>}
    {recordModal&&<WebcamVideoCapture recordHandler={setRecording} setRecordModal={setRecordModal}/>}
    {recording&&<VideoPreViewer recording={recording}/>}
    </div>
    </>
  )
}
